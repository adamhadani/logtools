...
def deploy(deploydir, virtualenv=None):
    ...
    run(source_me + 'easy_install -U {0}/{1}'.format(deploydir, env.dist_fname))  # noqa


def _find_dist():
    """Find latest version of our distributable and point to it in env"""
    _dist_fname = local("ls -tr dist/%s*.egg | tail -n1" % env.proj_name.replace("-", "_"))  # noqa
    if _dist_fname.failed is True:
        return -1
